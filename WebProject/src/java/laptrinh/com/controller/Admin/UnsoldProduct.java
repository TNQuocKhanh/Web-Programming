...
@WebServlet(name = "UnsoldProduct", urlPatterns = {"/unsold-product"})
public class UnsoldProduct extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (u.getRole() == 1) {
            ...
            List<Product> list = itemdao.ProductUnsold();
            long sumunsold=itemdao.countUnsoldProduct();
            request.setAttribute("sumunsold", sumunsold);
            request.setAttribute("list", list);

            request.getRequestDispatcher("unsold.jsp").forward(request, response);
        } else {
